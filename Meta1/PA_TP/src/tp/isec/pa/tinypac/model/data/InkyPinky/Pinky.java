...
import tp.isec.pa.tinypac.model.data.IMazeElement;
...
public class Pinky implements IGhost {
        ...
        for (Direction it:possibleDirections){
            ...
            switch (it){
                case UP -> {
                    ...
                    move= new int[]{x, y - 1};
                    setOverItem(game.getTrueBoard().get(y-1,x).getSymbol());
                    System.out.println(game.getTrueBoard().get(y-1,x).getSymbol());
                }
                case DOWN -> {
                    direction= UP;
                    dist=calculateDistanceToNextCorner(x, y+1);
    ...
    public char nextCell(GameBWData game,Direction it){
        char c;
